...
local function loadConfig()
  ...
  if not config then
    ...
    if root and not root.isReadOnly() then
      ...
      local f = fs.open("/etc/edit.cfg", "w")
      if f then
        local serialization = require("serialization").lua
        for k, v in pairs(env) do
          f:write(k.."="..tostring(serialization.encode(v, math.huge)).."\n")
        end
        f:close()
      end
    end
...
local function getCursor()
  local cx, cy = cursor.get().x, cursor.get().y
...
local function setCursor(nbx, nby)
  ...
  cursor.set(cursor.get().x, nby - scrollY)
  ...
  cursor.set(nbx - scrollX, nby - scrollY)
  ...
  --locstring = text.padLeft(locstring, 10)
    while unicode.wlen(locstring) < 10 do
        locstring = " " .. locstring
    end
...
local function delete(fullRow)
  local _, cy = cursor.get().x, cursor.get().y
  local cbx, cby = getCursor()
  local x, y, w, h = getArea()
...
local function find()
  ...
  while running do
    ...
    cursor.set(7 + unicode.wlen(findText), h + 1)
    ...
    if type(keybinds) == "table" and keyBindHandlers[command] then
      for _, keybind in ipairs(keybinds) do
        if type(keybind) == "table" then
          ...
          local keyboardAddress = keyboard.getAddress()
...
while running do
  local event, address, arg1, arg2, arg3 = require("event").pull()
  --if address == term.keyboard() or address == term.screen() then
  if event == "key_down" or event == "clipboard" or event == "touch" or event == "drag" or event == "scroll" then
    local blink = true
